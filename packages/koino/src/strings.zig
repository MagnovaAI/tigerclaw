...
// Named HTML entity lookup. Full CommonMark wants hundreds of these
// (`&amp;`, `&mdash;`, `&nbsp;`, ...) but the chat corpus is LLM
// output that almost never produces named entities — they're a
// product of HTML rendering, not markdown authoring. We keep only
// the handful that can show up in LLM prose and fall back to null.
const htmlentities = struct {
    const Entity = struct { characters: []const u8 };

    fn lookup(key: []const u8) ?Entity {
        // `key` arrives framed as `&name;` including both delimiters.
        ...
        for (named) |n| {
            if (std.mem.eql(u8, n.k, key)) return Entity{ .characters = n.v };
        }
        return null;
    }
};
...
pub fn normalizeLabel(allocator: mem.Allocator, s: []const u8) ![]u8 {
    ...
    while (it.nextCodepoint()) |cp| {
        if (isUnicodeWhitespace(cp)) {
            ...
            try encodeUtf8Into(if (cp < 128) @as(u21, std.ascii.toLower(@intCast(cp))) else cp, &buffer);
        }
    }
    return buffer.toOwnedSlice();
}
