...
Optional, build-flag-gated provider implementations. Each `extensions/<name>/root.zig`
...
Code under `extensions/<name>/` MAY import only:
...
that escape its own `extensions/<name>/` directory, and MAY NOT depend on
another extension.
...
`error: no module named '...'`. The previous symlink-based layout reduced this
to a docs-only convention; the named-module split makes it a hard guarantee.

## Adding a new extension

1. Drop `extensions/<name>/root.zig` exporting a provider type.
2. In `build.zig`: add an `enable_<name>` build option, create `ext_<name>`
